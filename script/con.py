...
def yamada_ode(x, t, perturbation, i):
    ...
    def pert(t):
        if i == 0 and t >= 300 and t <= 330:
            ...
            return 1
        elif i == 4 and t >= 300 and t <= 400:
            return 1
        elif i == 5 and t >= 300 and t <= 450:
            return 1
        elif i == 6 and t >= 300 and t <= 550:
            return 1
        elif i == 7 and t >= 300 and t <= 650:
            return 1
        elif i == 8 and t >= 300 and t <= 750:
            return 1
        elif i == 9 and t >= 300 and t <= 850:
            return 1
        else:
            return 0
...
style.use("seaborn")
plt.plot(tamp[0], I1, 'purple', label = '$t_p = 30$')
plt.plot(tamp[1], I2, 'blue', label = '$t_p = 40$')
plt.plot(tamp[2], I3, 'skyblue', label = '$t_p = 50$')
plt.plot(tamp[3], I4, 'greenyellow', label = '$t_p = 60$')
plt.plot(tamp[4], I5, 'yellow', label = '$t_p = 70$')
plt.plot(tamp[5], I6, 'orange', label = '$t_p = 100$')
plt.plot(tamp[6], I7, 'crimson', label = '$t_p = 150$')
plt.plot(tamp[7], I8, 'red', label = '$t_p = 250$')
plt.plot(tamp[8], I9, 'brown', label = '$t_p = 350$')
plt.plot(tamp[9], I10, 'grey', label = '$t_p = 450$')
